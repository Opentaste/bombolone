...
HashTableCtrl = function($scope, $resource, $rootScope, $location) {
  var counter, hash_table_index, hash_table_new, hash_table_overview, hash_table_update, hash_table_view, params, __get_value, _init_hash_table;
  ...
  _init_hash_table = function() {
    return $scope.hash_table = {
      language: $rootScope.language,
      lan: $rootScope.lan
    };
  };
  if (hash_table_index || hash_table_overview || hash_table_view) {
  ...
  if (hash_table_update || hash_table_view) {
    $scope.title = "Update";
    $scope.update = true;
    _init_hash_table();
    if (hash_table_view) {
    ...
    $scope.ajaxHashTableGet.get(params, function(resource) {
  ...
  $scope.change_name_label = function(name_label) {
    return $scope.name_label = name_label;
  };
  $scope.remove_label = function(key) {
    return delete $scope.hash_map.value[key];
  };
  counter = 0;
  $scope.add_label = function() {
    ...
    return $scope.hash_map.value[key] = value;
  };
  ...
  $scope["new"] = function() {
    ...
    return $scope.ajaxHashTableNew.save(paramas, function(resource) {
  ...
  $scope.save = function() {
    ...
    paramas = __get_value(paramas);
    return $scope.ajaxHashTableUpdate.save(paramas, function(resource) {
      return $scope.show_message(resource);
    });
  };
  return __get_value = function(hash_map) {
    var code, key, value, _i, _len, _ref, _ref1;
    _ref = $scope.list_code;
